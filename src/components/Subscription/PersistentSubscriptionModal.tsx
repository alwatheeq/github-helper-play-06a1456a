      ...
      <ScholarCard variant="elevated" padding="none" className="relative max-w-lg w-full overflow-hidden animate-scaleIn">
        <div className="bg-accent-gold p-6 text-ink-on-dark relative">
          ...
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div>
              ...
              <p className="text-ink-on-dark/90 text-sm">Unlock unlimited access</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="space-y-3 mb-6">
            <p className="text-sm font-semibold text-secondary-ink dark:text-muted-ink-on-dark">
              Premium includes:
            </p>
            {benefits.map((benefit, index) => (
              <div key={index} className="flex items-start space-x-3">
                <CheckCircle className="h-5 w-5 text-green-500 flex-shrink-0 mt-0.5" />
                <span className="text-sm text-secondary-ink dark:text-muted-ink-on-dark">{benefit}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col space-y-3">
            ...
            <button
              onClick={onDismiss}
              className="w-full py-2 px-4 text-secondary-ink dark:text-muted-ink-on-dark hover:text-ink dark:hover:text-ink-on-dark transition text-sm font-medium"
            >
              Maybe Later
            </button>
          </div>
