...
export const BookModeViewer: React.FC<BookModeViewerProps> = ({
  ...
  return (
    <div className={`min-h-screen bg-page-light dark:bg-page-dark relative overflow-hidden`}>
      ...
      {onClose && (
        <div
          className="absolute inset-0 bg-page-light dark:bg-page-dark bg-opacity-20 z-0"
          onClick={onClose}
        />
      )}
            ...
            <div className="p-1.5 space-y-0.5">
              {!widgets.some(w => w.type === 'book') && (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm text-secondary-ink dark:text-muted-ink-on-dark hover:bg-black/5 dark:hover:bg-white/5 rounded-[12px] transition-colors`}
                >
