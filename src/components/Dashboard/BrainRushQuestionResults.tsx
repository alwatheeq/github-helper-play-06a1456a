...
export const BrainRushQuestionResults: React.FC<BrainRushQuestionResultsProps> = ({
  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto">
        <div className={`bg-card-light dark:bg-card-dark rounded-[12px] shadow-[0_1px_3px_0_rgba(0,0,0,0.08),0_1px_2px_0_rgba(0,0,0,0.06)] border-divider dark:border-divider-on-dark dark:shadow-[0_1px_3px_0_rgba(0,0,0,0.08),0_1px_2px_0_rgba(0,0,0,0.06)] dark: p-12 text-center`}>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-[12px]  p-8 border-2 border-green-500 dark:border-green-600">
        <div className="flex items-center justify-center space-x-3 mb-4">
          <CheckCircle className="h-12 w-12 text-green-600 dark:text-green-400" />
          <h2 className={`font-display text-[38px] font-semibold text-ink dark:text-ink-on-dark`}>Correct Answer</h2>
        </div>
        <p className="font-display text-[24px] font-bold text-green-700 dark:text-green-300 text-center">
          {correctAnswer}
        </p>
      </div>

      {/* Statistics */}
      <div className="bg-card-light dark:bg-card-dark rounded-[12px] shadow-[0_1px_3px_0_rgba(0,0,0,0.08),0_1px_2px_0_rgba(0,0,0,0.06)] border border-divider dark:border-divider-on-dark dark:shadow-[0_1px_3px_0_rgba(0,0,0,0.08),0_1px_2px_0_rgba(0,0,0,0.06)] dark: p-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="bg-green-50 dark:bg-green-900/20 rounded-[12px] p-4 text-center">
            <p className="font-display text-[38px] font-semibold text-green-600 dark:text-green-400">{correctCount}</p>
            <p className={`text-sm text-secondary-ink dark:text-muted-ink-on-dark`}>Correct</p>
          </div>
          <div className="bg-red-50 dark:bg-red-900/20 rounded-[12px] p-4 text-center">
            <p className="font-display text-[38px] font-semibold text-red-600 dark:text-red-400">{incorrectCount}</p>
            <p className="text-sm text-secondary-ink dark:text-muted-ink">Incorrect</p>
          </div>
          <div className={`bg-page-light dark:bg-page-dark rounded-[12px] p-4 text-center`}>
            <p className={`font-display text-[38px] font-semibold text-secondary-ink dark:text-muted-ink-on-dark`}>{unansweredCount}</p>
        ...
        <div className="space-y-2">
          {participantResults.map((result, index) => {
            ...
            return (
              <div
                key={result.id}
                className={`p-4 rounded-[12px] border-2 transition ${
                  isCurrentUser
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    {result.answer && (
                      <>
                        <p className={`font-display text-[24px] font-bold text-ink dark:text-ink-on-dark`}>
