-- Standard plan add-on columns and credit logic:
-- - Basic Standard (no add-ons): 1500 tools, 0 Zego
-- - Standard + Zego: 1500 tools, zego_hours_per_cycle * 100 (e.g. 10 hrs = 1000)
-- - refresh_billing_cycle and initialize_subscription_credits use add-on columns for standard tier

-- =====================================================================
-- STEP 1: Add add-on columns to subscriptions
-- =====================================================================
...
COMMENT ON COLUMN subscriptions.zego_hours_per_cycle IS 'Zego add-on hours per billing cycle for standard tier; 100 credits per hour (e.g. 10 = 1000 credits).';
COMMENT ON COLUMN subscriptions.chat_blocks_per_cycle IS 'AI chat add-on blocks per cycle for standard tier; 1 block = 100k tokens.';

-- =====================================================================
-- STEP 2: initialize_subscription_credits — standard uses add-ons; others 1500+1000
-- =====================================================================
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- Standard tier: Zego only if add-on (zego_hours_per_cycle > 0)
    IF v_subscription.subscription_tier = 'standard' THEN
      v_zego_credits := COALESCE(v_subscription.zego_hours_per_cycle, 0) * 100;
    END IF;
    -- Else keep defaults: v_tool_credits=1500, v_zego_credits=1000 for legacy tiers
  ELSE
    v_cycle_start := now();
  ...
  RAISE NOTICE '[CREDIT_INIT] Tool=%, Zego=% for user: %', v_tool_credits, v_zego_credits, p_user_id;
...
COMMENT ON FUNCTION initialize_subscription_credits IS 'Initialize/refill credits: Standard tier uses add-ons (1500 tools, Zego = zego_hours_per_cycle*100). Other tiers: 1500 tools + 1000 Zego.';
...
-- =====================================================================
-- STEP 3: refresh_billing_cycle_if_expired — handle standard tier + add-ons
-- =====================================================================
...
BEGIN
  ...
  -- Standard: Zego from add-on; others 1000
  IF v_subscription_tier = 'standard' THEN
    v_zego_credits := v_zego_hours * 100;
  ELSE
    v_zego_credits := 1000;
  END IF;
...
COMMENT ON FUNCTION refresh_billing_cycle_if_expired IS 'Refreshes billing cycle. Standard tier: Zego from zego_hours_per_cycle*100. Other paid tiers: 1500 tools + 1000 Zego.';

-- =====================================================================
-- STEP 4: safe_create_subscription — add optional add-on params
-- =====================================================================
...
BEGIN
  ...
  -- Token limit: standard = 520000 + chat_blocks*100000 (min 520000); others from get_token_limit_for_tier
...
COMMENT ON FUNCTION safe_create_subscription IS 'Safely create subscription. Optional p_zego_hours, p_chat_blocks for standard tier add-ons.';
