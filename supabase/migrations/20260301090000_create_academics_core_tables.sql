/*
  # Create Academics Core Tables

  1. New Tables
    - academics_topics: shared topic catalog
    - academics_courses: user-owned courses linked to a topic
    - academics_course_items: map course -> user_library_items
    - academics_course_quizzes: map course -> quiz_sessions

  2. Security
    - Enable RLS on all new tables
    - Topics: authenticated users can read and insert
    - Courses and mappings: only owner can read/write

  3. Notes
    - This migration provides DB scaffolding (DB-first) for Academics phases.
    - Existing quiz/study tables remain unchanged.
*/
